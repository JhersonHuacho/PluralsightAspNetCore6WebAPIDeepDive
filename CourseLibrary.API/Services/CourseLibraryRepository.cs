...
#pragma warning restore CS8603 // Possible null reference return.
    ...
    public async Task<PagedList<Author>> GetAuthorsAsync(AuthorsResourceParameter authorsResourceParameter)
    {
        ...
        if (!string.IsNullOrWhiteSpace(authorsResourceParameter.OrderBy))
        {
            // get property mapping dictionary
            var authorPropertyMappingDictionary = _propertyMappingService
                .GetPropertyMapping<AuthorDto, Author>();

            collection = collection.ApplySort(authorsResourceParameter.OrderBy,
                authorPropertyMappingDictionary);

            //if (authorsResourceParameter.OrderBy.ToLowerInvariant() == "name")
            //{
